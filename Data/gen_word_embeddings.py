...
try:
    ...
    with open(path + 'clean.documents.txt') as f, open(path + 'clean.queries.txt') as f1, open(path + 'corpus.txt', 'w') as f2:
        lines = f.readlines()
        for line in lines:
    ...
    model = Word2Vec(LineSentence(path + 'corpus.txt'), size=dim, window=5, min_count=1, workers=multiprocessing.cpu_count(),sg=1,iter=50)
